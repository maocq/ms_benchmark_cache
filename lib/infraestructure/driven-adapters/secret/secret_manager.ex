defmodule Fua.DrivenAdapters.Secrets.SecretManagerAdapter do
  @moduledoc """
  Adapter responsible for retrieving secrets from secrets manager
  """
  use GenServer
  alias Fua.Config.ConfigHolder
  ...
  def init(_async_init) do
    ...
    {:ok, get_initial_secret_rds()}
  end

  defp create_ets do
    :ets.new(:secret_manager_adapter_rds, [:named_table, read_concurrency: true])
  ...
  def handle_info(:get_secret, _args) do
    ...
    {:noreply, get_initial_secret_rds()}
  end

  defp get_initial_secret_rds() do
    if Mix.env() != :dev do

      secret_name = ConfigHolder.conf().secret_name_rds
      ...
      :ets.insert(:secret_manager_adapter_rds, {:secret, secret})
      secret
    end
  end
  ...
  def get_secret_rds() do
    if Mix.env() != :dev do
      case :ets.lookup(:secret_manager_adapter_rds, :secret) do
        [{_, secret}] -> secret
